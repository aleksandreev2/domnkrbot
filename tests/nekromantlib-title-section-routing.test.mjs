...
test('social title modules consume centralized tab changes instead of owning URL routing', async () => {
  ...
  for (const source of [comments, discussions, reviews]) {
    assert.match(source, /title-tab-change/);
    assert.doesNotMatch(source, /history\.(?:pushState|replaceState)\(/);
  }
  assert.doesNotMatch(discussions, /DISCUSSION_SECTION_QUERY/);
});
