...
class Statement {
  ...
  async first() {
    ...
    if (this.query.includes("SUM(CASE WHEN r.release_kind = 'chapters' THEN r.chapter_count ELSE 0 END)")) return { count: this.db.pendingChapters };
...
test('stack title card queries pending chapter sum while instant card omits it', async () => {
  ...
  await withTelegram(async (calls) => {
    ...
    assert.ok(db.queries.some((q) => q.kind === 'first' && q.query.includes("SUM(CASE WHEN r.release_kind = 'chapters' THEN r.chapter_count ELSE 0 END)")));
  });
  ...
  await withTelegram(async (calls) => {
    ...
    assert.equal(db.queries.some((q) => q.query.includes("SUM(CASE WHEN r.release_kind = 'chapters' THEN r.chapter_count ELSE 0 END)")), false);
  });
});
