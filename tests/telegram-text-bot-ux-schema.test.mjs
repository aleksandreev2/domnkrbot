...
class Statement {
  ...
  async run() {
    ...
    if (this.db.duplicateInputActiveColumn && /ALTER TABLE telegram_proposal_sessions ADD COLUMN input_active/i.test(this.query)) {
      throw new Error('duplicate column name: input_active');
    }
...
class DB {
  constructor({ duplicateReturnColumn = false, duplicateInputActiveColumn = false } = {}) {
    this.queries = [];
    this.duplicateReturnColumn = duplicateReturnColumn;
    this.duplicateInputActiveColumn = duplicateInputActiveColumn;
  }
  prepare(query) { return new Statement(this, query); }
}

test('0017 is forward-only and adds proposal review-return, paused-input state and notification search state', async () => {
  ...
  assert.match(normalize(sql), /ALTER TABLE telegram_proposal_sessions ADD COLUMN input_active INTEGER NOT NULL DEFAULT 0/i);
  assert.match(sql, /CHECK \(input_active IN \(0, 1\)\)/i);
...
test('runtime UX schema repair creates proposal input state and search state without touching delivery triggers', async () => {
  ...
  assert.ok(db.queries.some((query) => /ALTER TABLE telegram_proposal_sessions ADD COLUMN input_active/i.test(query)));
...
test('runtime UX schema repair tolerates already-existing proposal UX columns independently', async () => {
  const { ensureTelegramTextBotUxSchema } = await loadSchema();
  const db = new DB({ duplicateReturnColumn: true, duplicateInputActiveColumn: true });
