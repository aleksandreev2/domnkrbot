...
test('discovery queues a final scan while completed titles are already hidden from active UX', () => {
  ...
  assert.match(scheduler, /is_active\s*=\s*CASE[\s\S]*translation_is_completed[\s\S]*THEN\s+0/i);
});

test('fast scanner prioritizes pending completions even when logically inactive and finalizes only after chapter polling succeeds', () => {
  ...
  assert.match(scanner, /translation_completion_pending\s*=\s*1\s+OR\s+\(is_active\s*=\s*1[\s\S]*snapshot_ready\s*=\s*0[\s\S]*notification_subscriber_count\s*>\s*0/i);
