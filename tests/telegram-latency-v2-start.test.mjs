...
class Statement {
  ...
  async run() {
    ...
    if (housekeeping) {
      this.db.events.push(`housekeeping:${this.query.split(' ').slice(0, 4).join(' ')}`);
...
test('/start begins Telegram menu output while required housekeeping is still pending', async () => {
  const db = new DB();
  ...
  try {
    ...
    assert.ok(db.events.some((event) => event.startsWith('housekeeping:')), 'housekeeping must have started');
    assert.ok(db.events.includes('telegram:sendMessage'), `menu send must start before housekeeping resolves; events=${JSON.stringify(db.events)}`);
    assert.equal(settled, false, 'handler must still await required housekeeping before returning');
