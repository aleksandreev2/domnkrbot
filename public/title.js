(() => {
  ...
  function renderChapters(){const host=$('#chapterList');if(!host||!state.data)return;const query=($('#chapterSearch')?.value||'').trim().toLowerCase();let chapters=[...(state.data.chapters||[])];if(query)chapters=chapters.filter((chapter)=>`${chapter.number||''} ${chapter.name||''} ${chapter.volume||''}`.toLowerCase().includes(query));chapters.sort((a,b)=>Number(a.chapter_id)-Number(b.chapter_id));if(state.sort==='new')chapters.reverse();const volumes=new Map();for(const chapter of chapters){const volume=String(chapter.volume||'Без тома');if(!volumes.has(volume))volumes.set(volume,[]);volumes.get(volume).push(chapter);}const read=readSet();host.innerHTML=chapters.length?Array.from(volumes.entries()).map(([volume,items])=>`<section class="volume-block"><div class="volume-head"><strong>${volume==='Без тома'?'Без тома':`Том ${esc(volume)}`}</strong><small>${items.length} глав</small></div>${items.map((chapter)=>chapterRow(chapter,read)).join('')}</section>`).join(''):'<div class="surface title-loading"><i data-lucide="search-x"></i><strong>Главы не найдены</strong><span>Измените номер или название в поиске.</span></div>';refreshIcons();}
