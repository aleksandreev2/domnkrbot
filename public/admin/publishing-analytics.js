(() => {
  ...
  async function openStatistics(){
    ...
    $('#pageTitle').textContent='Статистика';$('#pageDescription').textContent='Публикации, скачивания через бота, «Спасибо» и поддержка.';
    $('#adminContent').innerHTML=`<div class="admin-loading">${icon('loader-circle')} Собираем статистику…</div>`;refreshIcons();
    loading=true;
    try{const data=await api(`/api/admin/publishing-analytics?days=${days}`);if(active)paint(data);}catch(error){if(active)$('#adminContent').innerHTML=`<div class="notice error">${esc(error.message)}</div>`;}finally{loading=false;}
  }

  function paint(data){
    const s=data.summary||{};const period=days===0?'за всё время':`за ${days} дней`;
