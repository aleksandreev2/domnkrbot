(() => {
  ...
  async function renderSync(){
    const [data,authData]=await Promise.all([api('/api/ranobelib'),api('/api/admin/ranobelib/auth')]);
    const auth=authData.auth||{state:'missing'};
    const authLabels={missing:'не настроена',active:'активна',expired:'истекла',invalid:'недействительна',unavailable:'ключ недоступен'};
    $('#adminContent').innerHTML=`
      <div class="admin-stat-grid">${stat('▤',data.stats?.activeTitles||0,'активных тайтлов','orange')}${stat('✓',data.stats?.syncedTitles||0,'синхронизировано','green')}${stat('↗',data.stats?.releases||0,'релизов','gold')}${stat('↻',data.sync?.syncing?'…':'OK','состояние','blue')}</div>
      <div class="ranobelib-admin-grid">
        <section class="admin-panel"><div class="admin-panel-head"><div><h2>RanobeLib sync</h2><p>Последний запуск: ${dateTime(data.sync?.lastSyncAt)}</p></div><button id="runSync">Синхронизировать</button></div>${data.sync?.lastError?`<div class="notice error">${esc(data.sync.lastError)}</div>`:'<div class="notice">Ошибок последней синхронизации нет.</div>'}</section>
        <section class="admin-panel"><div class="admin-panel-head"><div><h2>Авторизация RanobeLib</h2><p>Для чтения скрытых 18+ тайтлов.</p></div><span class="admin-badge ${auth.state==='active'?'published':auth.state==='missing'?'draft':'failed'}">${esc(authLabels[auth.state]||auth.state)}</span></div>
          <div class="ranobelib-auth-meta"><span>Истекает: <b>${dateTime(auth.accessExpiresAt)}</b></span><span>Проверена: <b>${dateTime(auth.lastValidatedAt)}</b></span><span>Обновлена: <b>${dateTime(auth.lastRefreshedAt)}</b></span></div>
          ${auth.lastError?`<div class="notice error">${esc(auth.lastError)}</div>`:''}
          <form id="ranobelibAuthForm" autocomplete="off">
            <label class="admin-field"><span>Access token</span><input id="ranobelibAccessToken" type="password" autocomplete="off" required></label>
            <label class="admin-field"><span>Refresh token</span><input id="ranobelibRefreshToken" type="password" autocomplete="off" required></label>
            <label class="admin-field"><span>Срок access token</span><input id="ranobelibExpiresAt" type="datetime-local" required></label>
            <div class="publisher-actions"><button class="primary" type="submit">${auth.state==='missing'?'Подключить':'Заменить авторизацию'}</button>${auth.state!=='missing'?'<button id="removeRanobelibAuth" class="bad" type="button">Удалить</button>':''}</div>
          </form>
        </section>
      </div>`;
    $('#runSync').addEventListener('click',async()=>{const button=$('#runSync');button.disabled=true;button.textContent='Синхронизация…';try{await api('/api/admin/ranobelib/sync',{method:'POST'});toast('Синхронизация завершена.');await renderSync();}catch(error){toast(error.message,true);button.disabled=false;}});
    $('#ranobelibAuthForm').addEventListener('submit',async(event)=>{event.preventDefault();const access=$('#ranobelibAccessToken'),refresh=$('#ranobelibRefreshToken'),expiry=$('#ranobelibExpiresAt');try{await api('/api/admin/ranobelib/auth',{method:'PUT',headers:{'content-type':'application/json'},body:JSON.stringify({accessToken:access.value,refreshToken:refresh.value,expiresAt:new Date(expiry.value).toISOString()})});toast('Авторизация RanobeLib сохранена.');await renderSync();}catch(error){toast(error.message,true);}finally{access.value='';refresh.value='';expiry.value='';}});
    $('#removeRanobelibAuth')?.addEventListener('click',async()=>{if(!confirm('Удалить авторизацию RanobeLib?'))return;try{await api('/api/admin/ranobelib/auth',{method:'DELETE'});toast('Авторизация RanobeLib удалена.');await renderSync();}catch(error){toast(error.message,true);}});
  }
